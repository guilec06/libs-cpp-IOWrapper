#ifndef IOPROMISE_HPP_
#define IOPROMISE_HPP_
...
class IOPromise {
    public:
        ...
        static std::shared_ptr<IOPromise<T>> create(Executor&& executor)
        {
            auto promise = std::make_shared<IOPromise<T>>();
            executor(
        ...
        IOPromise& then(std::function<void(T&)> cb)
        {
        ...
        IOPromise& catch_(std::function<void(int)> cb)
        {
            std::unique_lock<std::mutex> lock(mutex_);
        ...
        IOPromise& finally(std::function<void(T&, int)> cb)
        {
            std::unique_lock<std::mutex> lock(mutex_);
            if (state != State::Pending) {
                lock.unlock();
